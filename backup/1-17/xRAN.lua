...
--base mac layer protocol
 ...
 do
	...
	function parse_CplaneSectionExtension(_root, _value, _len)
		...
		while ef == 1 do
			sub_node = _root:add(roe_proto.fields[13], _value:range(offset, 2))
			...
			sub_node:add(roe_proto.fields[14], _value:range(offset + 2, extLen - 2))
	...
	function parse_uPlane_Prbu(_root, _value, _len)
		...
		local sub_node = _root:add(roe_proto.fields[18], _value:range(offset, prbu_len))
	...
	function parse_UplaneSection(_root, _value, _len)
		...
		local sub_node = _root:add(roe_proto.fields[17], _value:range(offset, 6))
	...
	function parse_UplaneSections(_type, _numberOfsections, _root, _value, _len)
		...
		while (offset < _len and index < _numberOfsections) do	
			local sub_node = _root:add(roe_proto.fields[16], _value:range(offset, 6))
    ...
    function xran_proto_dissector_one(_roeSubType, _root, _value, _len) 
		...
		local sub_node = node:add(roe_proto.fields[10], common_header_part)
		...
		if (_roeSubType == 130 or _roeSubType == 131) then
			...
			if sectionType >= 0 and sectionType <= 7 then
				sub_node = node:add(roe_proto.fields[11], plane_sections_buf)
			...
			sub_node = node:add(roe_proto.fields[15], plane_sections_buf)
    ...
    function xran_proto_dissector(_roeSubType, _pkt, _root, _value, _len) 
