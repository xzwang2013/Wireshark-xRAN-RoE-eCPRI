...
do
	...
	local xran_common_header_type     = ProtoField.bytes("RoE.xran_header", "Common Header", base.HEX)	
	local xran_cplane_sections    = ProtoField.bytes("RoE.cplane_sections", "Cplane Sections", base.HEX)	
	local xran_cplane_section    = ProtoField.bytes("RoE.cplane_section", "Section", base.HEX)	
	local xran_cplane_extension    = ProtoField.bytes("RoE.cplane_extension", "Extension", base.HEX)
	local xran_cplane_section_extension_data    = ProtoField.bytes("RoE.cplane_section_extension_data", "Data", base.HEX)
	
	local xran_uplane_sections    = ProtoField.bytes("RoE.uplane_sections", "Uplane Sections", base.HEX)	
	local xran_uplane_section    = ProtoField.bytes("RoE.uplane_section", "Section", base.HEX)	
	local xran_uplane_header    = ProtoField.bytes("RoE.uplane_header", "Uplane Header", base.HEX)	
	local xran_uplane_prbu    = ProtoField.bytes("RoE.uplane_prbu", "Prbu Data", base.HEX)	
    ...
    roe_proto.fields = {
		...
		roe_tlv_value,
        roe_content,
		xran_common_header_type, --10
		xran_cplane_sections, --11
		xran_cplane_section, --12
		xran_cplane_extension, --13
		xran_cplane_section_extension_data, --14
		xran_uplane_sections, --15
		xran_uplane_section, --16
		xran_uplane_header, --17
		xran_uplane_prbu, --18
		spirent_tag
    }
...
--]]
		...
		elseif _sub_type >= 128 and _sub_type <= 131 then
            ...
            xran_proto_dissector(_sub_type, pkt, g_roe_tree, buf(offset), xran_content_length)
